...
def viewVotes(request):
    ...
    for vote in votes:
        # Hash the voter's name using SHA-256
        hashed_name = hashlib.sha256((vote.voter.admin.first_name + vote.voter.admin.last_name).encode()).hexdigest()
        # Create a tuple with the hashed name and other details
        hashed_vote = (hashed_name, vote.candidate, vote.position)
        # Append the hashed vote to the list
        hashed_votes.append(hashed_vote)
